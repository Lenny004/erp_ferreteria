# Ferreteria - Plan Validado de Finalizacion de la Aplicacion
...
> **Base revisada:** `README.md`, `schema.prisma` v3.0, vistas WPF existentes, modelos EF Core, estandares `FERRETERIA_C_CODING_STANDARDS_2026.md`, modulo empleados/planilla de `beraka-core-api` y análisis comparativo propuesta ERP FERRESAL (COT-9520).
...
- **Módulos descartados para MVP:** Cuentas por Cobrar (CxC) — Ferreteria opera principalmente al contado; puede incorporarse en una fase futura si se requiere crédito a clientes.
...
| Repos administracion | `ferreteria_backend` (API) + `Ferreteria-adminweb` (frontend) |
...
| **CxC (Cuentas por Cobrar)** | **Descartada para MVP.** Ferreteria opera al contado. El modelo `Customer` existe como base si se requiere credito en el futuro. |
...
Estas vistas existen fisicamente en `Ferreteria.PuntoVenta/Views`:
...
`dotnet build Ferreteria.PuntoVenta/Ferreteria.PuntoVenta.csproj` compila correctamente con 0 errores y 0 advertencias.
...
El esquema actual `hr` cubre empleados basicos y una planilla mensual simplificada, pero **no soporta** el control de planilla que Ferreteria requiere en `Ferreteria-adminweb`. Comparado con la referencia `beraka-core-api`:
...
Basado en `FERRETERIA_C_CODING_STANDARDS_2026.md`:
...
- Seeds idempotentes.
- Validacion de `FerreteriaDbContext` contra schema real.
...
| Seeds RRHH | Bancos, documentos, ISR, feriados, puestos/departamentos Ferreteria |
| Prisma schema | `multiSchema` con `@@map` PascalCase en `ferreteria_backend` |
| EF Core | Modelos C# con `Guid` para PKs/FKs; WPF solo consulta/escribe datos operativos de caja (ver 17.12) |
| Documentacion | Mapeo Beraka → Ferreteria en seccion 17.5 |

**Regla operativa Fase 0 — fuente de verdad BD:** desde v3.0, `ferreteria_backend/prisma/schema.prisma` es la fuente principal del schema. `Ferreteria.PuntoVenta/Squema.sql` queda como referencia legacy hasta eliminarlo o convertirlo en migraciones raw puntuales. No ejecutar ambos caminos sobre la misma BD.
...
**Prioridad:** Alta  
**Esfuerzo:** Alto  
**Objetivo:** Crear el sistema web publico/administrativo de Ferreteria.
...
- CRUD de proveedores en Ferreteria-adminweb.
...
- Planilla **quincenal** (operacion principal Ferreteria) + soporte **mensual** y **semanal**.
...
**Implementacion:**
- Endpoints de agregacion en ferreteria_backend: GET /api/v1/dashboard/sales, /inventory, /purchases, /hr.
- Sin tablas adicionales; todas las consultas sobre datos existentes con indices adecuados.
- Filtros por rango de fechas.
- Frontend: pagina /dashboard en Ferreteria-adminweb con tarjetas, graficas (recharts o chart.js) y tablas.
...
```text
ferreteria_backend/          # Node.js 22+ + Express 5 + Prisma 6 — API administrativa
Ferreteria-adminweb/         # Next.js 15 + React 19 + Tailwind CSS 4 — UI administrativa
Ferreteria.PuntoVenta/     # WPF caja — EF Core directo a Supabase (decision hibrida)
```
...
### 7.2 Estructura `ferreteria_backend`
...
### 7.3 Estructura `Ferreteria-adminweb`
...
|   +-- features/
|   +-- lib/
|   |   +-- api.ts               # Cliente REST hacia ferreteria_backend
...
**No usar Prisma directo desde adminweb** si el backend centraliza reglas, auditoria e importaciones Excel. El frontend debe consumir `ferreteria_backend` por HTTP.
...
Cada commit debe ser atomico, verificable y alineado con `FERRETERIA_GIT_PR_2026.md`.

### 8.1 Convencion Gitmoji para Ferreteria

Usar Gitmoji de forma consistente. No elegir emojis por gusto visual; el emoji debe describir el tipo principal del cambio.

| Emoji | Uso en Ferreteria | Ejemplo |
|---|---|---|
...
| 16 | `🎉 Se crea la base de los repositorios ferreteria_backend y Ferreteria-adminweb.` | Scaffold Node/Next |
...
| — | .NET | `net10.0-windows` |
| — | Repos web | `ferreteria_backend` + `Ferreteria-adminweb` |
...
11. `ferreteria_backend` y `Ferreteria-adminweb` seran repos separados o un monorepo con `apps/api` y `apps/admin`?
...
| QA-18 | **Esquema planilla simplificado** | No cumple requerimiento RRHH Ferreteria | Fase 0b + seccion 17 |
...
**Libreria recomendada:** ExcelJS o SheetJS/xlsx en `ferreteria_backend` (no COM Excel en servidor; no requiere Office instalado).

**Flujo:**

1. Admin sube archivo en `Ferreteria-adminweb` hacia `ferreteria_backend` → `/api/v1/importaciones`.
...
**Entregables de ese modulo (a replicar en ferreteria_backend):**
...
2. Aplicar schema Prisma v3.0 desde `ferreteria_backend` (`npm run db:push` en desarrollo; `prisma migrate` cuando se congele migracion).
...
15. Crear/continuar `ferreteria_backend` (Node.js 22 + Express 5 + Prisma 6) y `Ferreteria-adminweb` (Next.js 15 + React 19).
...
> **Estado:** Planeacion confirmada por el equipo. **No incluye implementacion de codigo** en esta revision; define refactorizacion de BD, alcance funcional y referencia para `ferreteria_backend` + `Ferreteria-adminweb`.
>
...
Dotar a Ferreteria de un **sistema de control de planilla** en la WebApp administrativa, equivalente en capacidades al modulo empleados/planilla de `beraka-core-api`, sin afectar la operacion WPF de caja (PIN, ventas, sesiones de cajero).
...
|---|---|
| `ferreteria_backend` (Node) | CRUD completo RRHH, planilla, inventario admin, `system.WebUsers`, reportes |
| `Ferreteria-adminweb` (Next) | UI que consume **solo** la API Node (`/api/v1/...`) |
| `Ferreteria.PuntoVenta` (WPF) | Lectura/escritura **operativa**: ordenes, DTE, caja, PIN, stock en venta. **No** implementa login web, planilla ni CRUD de empleados |

### 17.3.1 `system.WebUsers` — solo administracion web (Node)

`WebUsers` es la tabla de **usuarios del panel administrativo** (login email/password + JWT en `ferreteria_backend`). **No pertenece al proyecto WPF en C#** y no se implementara alli.
...
El modelo `WebUser.cs` **no existe en el proyecto WPF**; `system.WebUsers` lo consume solo `ferreteria_backend` (Prisma).
...
**WPF (caja):** solo lee/escribe campos operativos del empleado (`PinHash`, `CanSell`, `CanCashier`, `IsActive`, nombre para UI). Expediente, bancos, documentos y planilla se administran **exclusivamente** en `ferreteria_backend` + `Ferreteria-adminweb`.

### 17.5 Mapeo de tablas: Beraka → Ferreteria (`hr`)

Convencion Ferreteria: esquema `hr`, tablas PascalCase entre comillas, columnas PascalCase.

| Beraka (Prisma) | Ferreteria (SQL propuesto) | Accion |
|---|---|---|
...
### 17.9 Planeacion backend (`ferreteria_backend`) — sin implementar
...
### 17.10 Planeacion adminweb (`Ferreteria-adminweb`) — sin implementar
...
| CRUD empleados, PINs, expediente | `ferreteria_backend` + adminweb | No — solo consume PIN ya hasheado |
| Planilla, aguinaldo, liquidaciones | `ferreteria_backend` | No |
| Inventario admin (entradas, ajustes) | `ferreteria_backend` | No |
| Login admin (`WebUsers`) | `ferreteria_backend` JWT | No — **no hay admin web en C#** |
